...
class run_mav_active_3d_planning():
    def __init__(self):
...
if __name__ == '__main__':
    runner = run_mav_active_3d_planning()
    time.sleep(0.2)
    ...
    while 1:
        try:
            ...
            #     else:
            #         print('masaka?')
            #         take_off_input = runner.cur_local_pose # PoseStamped()
            #         take_off_input.header.stamp = rospy.Time.now()
            #         take_off_input.pose.position.z = 1.0
            #         runner.pub_pose.publish(take_off_input)
